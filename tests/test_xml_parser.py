...
<RecognitionData>
  ...
  <Coordinates><Latitude>52.6395215</Latitude><Longitude>39.6612541667</Longitude></Coordinates>
...
def test_sign_filter_accepts_parking_violation_codes() -> None:
    assert extract_sign(SAMPLE_XML) == "1.01"
    ...
    assert not has_required_sign("1.1")
    assert not has_required_sign("1.01.7")
    assert not has_required_sign("invalid")
